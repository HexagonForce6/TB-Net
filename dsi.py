...
class TBNetDSI:
    ...
    def parse_function(self, filename, label):
        img_decoded = tf.compat.v1.image.decode_image(tf.compat.v1.io.read_file(filename), channels=3, expand_animations=False)
        # Crop 5% from each side, but 25% from the bottom
        img = tf.compat.v1.image.crop_to_bounding_box(img_decoded, 11, 11, 168, 202)
        img = tf.compat.v1.image.resize(img, [IMG_HEIGHT, IMG_WIDTH])
        img = tf.compat.v1.image.convert_image_dtype(img, dtype=tf.compat.v1.float32)/255.

        # Apply the mask onto the image, thus masking the corners
        img = tf.compat.v1.where(black_mask, img, tf.compat.v1.zeros_like(img))

        # Renormalize
        min_value = tf.compat.v1.reduce_min(img)  # min is 0 because of black masking
        max_value = tf.compat.v1.reduce_max(img)
        img = tf.compat.v1.divide(tf.compat.v1.subtract(img, min_value), max_value)

        # Next, apply the average mask to make the corners grayish instead of black
        img = tf.compat.v1.math.add(img, avg_mask)

        return img, tf.compat.v1.one_hot(label, NUM_CLASSES)
    ...
    def parse_function_train(self, filename, label):
        ...
        if random.random() < AUGMENTATION_CHANCE:
            ...
            if which_aug == 0:
                img = tf.compat.v1.image.random_crop(img, [202,202,3])
                img = tf.compat.v1.image.resize_images(img, [224,224])
            elif which_aug == 1:
                img = tf.compat.v1.image.random_flip_left_right(img)
            elif which_aug == 2:
                img = tf.compat.v1.image.random_brightness(img, 0.1)
            elif which_aug == 3:
                img = tf.compat.v1.image.random_contrast(img, 0, 0.2)
        ...
        # Renormalize
        min_value = tf.compat.v1.reduce_min(img)
        max_value = tf.compat.v1.reduce_max(img)
        img = tf.compat.v1.divide(tf.compat.v1.subtract(img, min_value), max_value)
    ...
    def get_split(self, csv_path, phase="train", num_shards=1, shard_index=0):
        ...
        dataset = tf.compat.v1.data.Dataset.from_tensor_slices((np.array(data_x), np.array(data_y)))
        num_data = len(data_y)
